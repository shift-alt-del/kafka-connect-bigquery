...
## Customized changes from original code

- Renamed connector class to `com.wepay.kafka.connect.bigquery.BigQuerySinkConnectorX` to avoid conflict with the original connector.
- Added `intermediateDataset` configuration to allow collecting intermediate tables into the same datasets, which may bring a better table view in BigQuery UI. 
The connector writes intermediate tables to the same dataset of destination dataset by default when leave this configuration as `null`. 
